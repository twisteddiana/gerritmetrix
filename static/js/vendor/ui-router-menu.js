        ...
        return {
            link: {
                ...
                pre: function link(scope, element, attrs) {
                    ...
                    scope[attrs.menus] = $menus.get(menuOptions, false);

                    scope.$on('changeState', function(event){
                        scope[attrs.menus] = $menus.get(menuOptions, false);
                    })
                }
            }
...
// Source: src/services/menus.js
    uiRouterMenusModule.service('$menus', ['$state', '$rootScope', function($state, $rootScope) {
        ...
        function compile(state) {
            ...
            if (!isAllowed(state)) return null;
         ...
         * @returns {Array}
         */
        this.get = function(options, account) {
                    ...
                    if(tags) { //filter menus for tags
                        if(isDefined(menu.tag)) { //only if menu has a tag, skip tag less menu items
                            if(matchAny(tags, menu.tag)) {
                                ...
                                var root_state = split[0]+"."+split[1];
                                //if (state.name.indexOf(root_state) == 0) {
                                    if ($state.current.name.indexOf(state.name) == 0)
                                        menu.active = true;
                                    else
                                        menu.active = false;
                                    menus.push(menu);
